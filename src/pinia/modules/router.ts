...
const routerListArr: Array<{ label: string; value: string }> = [];
const keepAliveRoutersArr: Array<string> = [];
...
const formatRouter = (
  ...
  routes.forEach(item => {
    ...
    if (item.name) routeMap[item.name] = item;
...
const KeepAliveFilter = (routes: Array<RouteRecordRaw>) => {
  routes.forEach(item => {
    ...
    if (
      ...
      if (item.component) {
        item.component = item.component as () => Promise<
          typeof import('*.vue')
        >;
...
export const useRouterStore = defineStore('router', () => {
  const asyncRouters = ref<Array<RouteRecordRaw>>([]);
